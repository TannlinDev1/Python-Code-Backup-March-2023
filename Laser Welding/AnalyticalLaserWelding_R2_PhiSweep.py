...
v = 20e-3 #welding velocity (m/s)

dz = 10e-6 #mesh step size (m)
z = 1e-3 #mesh total size (m)
...
r_ft = 0.415e-3 #Spot radius at top of material (m)
...
RAD = np.zeros((N_PHI, N_z_steps))
MELT_RAD = np.zeros((N_PHI, N_z_steps))

for iter in range(0, N_PHI):

    print("" + str(np.round(iter * 100 / N_PHI, 1)) + " %")  # print progress
    ...
    RAD[iter,:] = rad2
    MELT_RAD[iter,:] = melt_rad2

X_KR = np.zeros((N_PHI, N_z_steps))
Y_KR = np.zeros((N_PHI, N_z_steps))
X_MR = np.zeros((N_PHI, N_z_steps))
Y_MR = np.zeros((N_PHI, N_z_steps))
X_L = np.zeros((N_PHI, N_z_steps))
Y_L = np.zeros((N_PHI, N_z_steps))

for J in range(0, N_z_steps):
    X_KR[:, J], Y_KR[:, J] = radtocart(RAD[:, J], PHI_ARR)
    X_MR[:, J], Y_MR[:, J] = radtocart(MELT_RAD[:, J], PHI_ARR)

# plt.figure(1)
# plt.plot(X_KR*1e3, Y_KR*1e3, label="Keyhole")
# plt.plot(X_MR*1e3, Y_MR*1e3, label="Melt")
# plt.plot(X_L*1e3, Y_L*1e3, label="Laser")
# plt.xlabel("X (mm)")
# plt.ylabel("Y (mm)")
# plt.xlim([-np.max(Y_MR*1000), np.max(Y_MR*1000)])
# plt.ylim([-np.max(Y_MR*1000), np.max(Y_MR*1000)])

fig = plt.figure()
ax = fig.add_subplot(projection = "3d")
J = 0
for J in range(0, N_PHI):
    ax.scatter(X_KR[J,:]*1e3, Y_KR[J,:]*1e3, z_arr*1e3, color="tab:orange")
    ax.scatter(X_MR[J,:]*1e3, Y_MR[J,:]*1e3, z_arr*1e3, color="tab:blue")
...
ax.set_zlim(0, 1.5)
ax.legend(["Keyhole", "Melt"])
ax.title("P = " +str(P_L)+ " W, v = " +str(np.round(v*1000, 0))+ " mm/s")
#
...
# plt.legend()
# plt.grid()
# plt.title("P = " +str(P_L)+ " W, v = " +str(np.round(v*1000, 0))+ " mm/s")
#
