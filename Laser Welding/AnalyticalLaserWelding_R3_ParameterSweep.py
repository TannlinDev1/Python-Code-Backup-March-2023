...
#Uses a heat balance between laser power and conductive losses to solve for radius at which
#steel reaches vaporisation and melting temperature.

#Assumes top wall angle is 45 deg then iteratively solves for wall angle throughout material thickness.

#Secant method solver is used and initialisation function is used for calculating keyhole radius

#Keyhole radius is solved for twice, including effect of multiple reflections inside keyhole the 2nd time

#Laser power and translational velocity are varied for sensitivity analysis
...
V_GLOB = np.linspace(V_MIN, V_MAX, N_GLOB)
P_L_GLOB = np.linspace(P_L_MIN, P_L_MAX, N_GLOB)

D_P = np.zeros((N_GLOB, N_GLOB))
W_T = np.zeros((N_GLOB, N_GLOB))
W_B = np.zeros((N_GLOB, N_GLOB))
...
for glob_I in range(0, N_GLOB):
    ...
    for glob_J in range(0, N_GLOB):
        ...
        v = V_GLOB[glob_I]
        P_L = P_L_GLOB[glob_J]
        r_ft = 0.625e-3#Spot radius at top of weld (m)
        ...
        lambda_th = 12.7 #thermal conductivity of steel (W/mK)
        ...
        if rad[0] == 0:
            ...
            W_T[glob_I, glob_J] = None
            W_B[glob_I, glob_J] = None

        else:

            n_mr = np.pi/(4*np.nanmean(theta)) #mean number of reflections of incident beam (need to double to give better weld results, don't know why yet)
            ...
            d_p = z_arr[np.where(rad2 == np.nanmin(rad2))]
            ...
            if rad2[0] == 0:
                ...
                D_P[glob_I, glob_J] = d_p*1e3
                W_T[glob_I, glob_J] = melt_rad2[0]*2e3
                W_B[glob_I, glob_J] = np.interp(t, z_arr, melt_rad2)*2e3

os.chdir(r"C:\Users\angus.mcallister\Documents\TFF\Welding Sim Results\Sim Results R4")
np.savetxt("Penetration Depth.txt", D_P, delimiter=" ",fmt='%f')
np.savetxt("Top Melt Width.txt", W_T, delimiter=" ",fmt='%f')
np.savetxt("Bottom Melt Width.txt", W_B, delimiter=" ",fmt='%f')
