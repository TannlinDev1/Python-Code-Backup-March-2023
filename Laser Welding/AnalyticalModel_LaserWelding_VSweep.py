...
#This simulation performs a sensitivity analysis of the model w.r.t welding speed

#Processing Parameters

v_min = 30e-3 #minimum weld speed (m/s)
v_max = 65e-3 #maximum weld speed (m/s)
N_GLOB = 100
v_arr = np.linspace(v_min, v_max, N_GLOB) #velocity array

D_GLOB = np.zeros((N_GLOB,2))
R_K_GLOB = np.zeros((N_GLOB,2))
W_T_GLOB = np.zeros((N_GLOB,2))
W_B_GLOB = np.zeros((N_GLOB,2))
...
for glob_I in range(0, N_GLOB):
    ...
    for J in range(0, N_phi): #Solve for keyhole dimensions across full revolution and convert to cartesian co-ordinates
        ...
        # print(np.round(J*100/N_phi,0)) #print progress
    ...
    D_GLOB[glob_I,1] = dstar
    R_K_GLOB[glob_I,1] = R_K
    W_T_GLOB[glob_I,1] = W_T
    W_B_GLOB[glob_I,1] = W_B

    print("" +str(np.round(glob_I*100/N_GLOB))+ " %")
...
plt.figure(1)
plt.plot(v_arr*1000, D_GLOB[:,1]*1000)
plt.xlabel("Welding Speed (mm/s)")
plt.ylabel("Penetration Depth (mm)")
plt.grid()
plt.title("P = " +str(np.round(P_L,2))+ " W, D = " +str(np.round(r_f*1000,2))+ " mm")

plt.figure(2)
plt.plot(v_arr*1000, R_K_GLOB[:,1]*1000)
plt.xlabel("Welding Speed (mm/s)")
plt.ylabel("Keyhole Radius (mm)")
plt.grid()
plt.title("P = " +str(np.round(P_L,2))+ " W, D = " +str(np.round(r_f*1000,2))+ " mm")

plt.figure(3)
plt.plot(v_arr*1000, W_T_GLOB[:,1]*1000, label="Top")
plt.plot(v_arr*1000, W_B_GLOB[:,1]*1000, label="Bottom")
plt.xlabel("Welding Speed (mm/s)")
plt.ylabel("Melt Width (mm)")
plt.grid()
plt.title("P = " +str(np.round(P_L,2))+ " W, D = " +str(np.round(r_f*1000,2))+ " mm")
plt.legend()

os.chdir(r"C:\Users\angus.mcallister\Documents\Laser Welding Sim Results\Welding Speed")
np.savetxt("Penetration Depth.txt", D_GLOB*1000, delimiter=",", fmt='%f')
np.savetxt("Keyhole Radius.txt", R_K_GLOB*1000, delimiter=",", fmt='%f')
np.savetxt("Top Melt Width.txt", W_T_GLOB*1000, delimiter=",", fmt='%f')
np.savetxt("Bottom Melt Width.txt", W_B_GLOB*1000, delimiter=",", fmt='%f')