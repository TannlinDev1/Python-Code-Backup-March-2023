...
P_L = 800 #Nominal output power (W)
r_ft = 0.7e-3 #Spot radius at top of material (m)
v = 10e-3 #welding velocity (m/s)
...
def secant_method_rk(th, r_f, T, v, lambda_th, N_rk_err, phi, a_mr): #secant method for solving rk error fcn == 0
    ...
    r_1 = r_f # initial upper estimate for rk - the evaporative zone cannot be outside of the focal radius however I've allowed for some wiggle room
...
# os.chdir(r"R:\TFF\Welding\Frame Weld Tests\Parameter Testing R3")
#
# XY_E = np.loadtxt(fname="P750V30Z58.3_S2_XYData.csv", delimiter=",")
# XY_E[:,0] = XY_E[:,0] - XY_E[0,0]
# XY_E[:,1] = XY_E[:,1] - XY_E[-1,1]
#
# XY_E1 = np.loadtxt(fname="P750V30Z58.3_S1_XYData.csv", delimiter=",")
# XY_E1[:,0] = XY_E1[:,0] - XY_E1[0,0]
# XY_E1[:,1] = XY_E1[:,1] - XY_E1[-1,1]
# XY_E1[:,1] *= -1
...
# plt.plot(XY_E[:-1,0], XY_E[:-1,1], label="Sample 1")
# plt.plot(XY_E1[:-1,0], XY_E1[:-1,1], label="Sample 2")
