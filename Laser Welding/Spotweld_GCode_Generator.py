...
x_0 = -65 # starting x position
y_0 = -65 # centreline of stitch pattern

width = 4.21 #width of filtered sections

x_step = 2 # x distance between spots
dy = 1
#y distance of spot from centreline

N = int(abs(x_0)*2/x_step + 1) #total number of points (before filter)
...
def GCode_Gen(points, N): #define G code generator function
    ...
    while I<N: #write G Code
        ...
        code[I, 0] = "G01    " + "X    " + str(np.round(points[J,1],2)) + "    Y    " + str(np.round(points[J,0],2))
        code[I+1,0] = "CALL WELD"

        if I >= N -2: #terminating statement for numerical error at end of iteration cycle
            break

        else:
            code[I+2, 0] = "G01    " + "X    " + str(np.round(points[J+1,1],2)) + "    Y    " + str(np.round(points[J+1,0],2))
            code[I + 3, 0] = "CALL WELD"
