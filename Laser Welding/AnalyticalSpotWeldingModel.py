...
def secant_method_th(r, r_f, T, lambda_th, N_th_err, t, a_mr): #secant method for solving theta error fcn == 0
    ...
    th_1 = np.pi/2  # initial upper estimate for th
...
def secant_method_rm(r_k, T_v, T_m, lambda_th, N_rm_err, t):  # secant method for solving rk error fcn == 0
    ...
    r_0 = r_k  # initial lower estimate for rm
    r_1 = r_k * 2.5  # initial upper estimate for rm
...
# LSS = line_source_strength(T_v, lambda_th, rad2[0], on_time)
# r_init = rad2[0]*1.2
# r_fin = rad2[0]*2
# r_arr = np.linspace(r_init, r_fin, 100)
# f_rt = np.zeros(100)
#
# for i in range(0, 100):
#     f_rt[i] = rm_err_fcn(LSS, T_m, lambda_th, r_arr[i], on_time)
#
# plt.plot(r_arr*1000, f_rt)
...
rad2, theta2, r_f2, melt_rad2 = keyhole_profile(np.pi/4, r_ft, T_v, T_m, lambda_th, N_rk_err, N_th_err, N_rm_err, on_time, dz, z, a_mr) #recalculate with multiple reflections
d_p = z_arr[np.where(rad2 == np.nanmin(rad2))]*1000

plt.plot(z_arr*1e3, rad*1e3, label="Keyhole")
plt.plot(z_arr*1e3, rad2*1e3, label="Keyhole Inc MR")
plt.plot(z_arr*1e3, melt_rad2*1e3, label="Melt Rad")
plt.xlabel("Z Axis (mm)")
plt.ylabel("R Axis (mm)")
plt.legend()
plt.grid()
plt.title("P = " +str(P_L)+ " W, t = " +str(np.round(on_time*1000, 0))+ " ms")
