...
V_g = 150 #Gas Velocity (m/s)
P_g = 1.51E6 + P_a #Gas Pressure (Pa)
rho_g = 15 #Gas Density (kg/m^3)
mu_g = 1.9e-5 #Gas Viscosity (kg/ms)
...
x_t = np.zeros(len(r_k))

Pe = np.zeros(len(r_k))
LHS_side = np.zeros(len(r_k))

R = np.zeros(len(r_k))
I_L_side = np.zeros(len(r_k))
Error_Side = np.zeros(len(r_k))
...
def find_geometry():
                for j in range(len(r_k)):
                        ...
                        x_t[j] = np.sqrt(r_k[j]*0.5*(1+(Z_0/z_r)**2)*np.log(0.37*np.tan(6*np.pi/180)*I_0*(1+(Z_0/z_r)**2)/q_front2[j]))
                        R[j] = np.sqrt((x_t[j]-r_k[j])**2+r_k[j]**2)#Radius of side of cylinder
                        I_L_side[j] = I_0*np.exp((-2*R[j]**2)/w**2)#Laser power intensity at side

                        LHS_side[j] = q_side2[j]/(0.41*np.tan(3*np.pi/180))#LHS of equation 5 in paper
        
                        Error_Side = abs(1-LHS_side[j]/I_L_side[j])#Define error at side
                        
                        if Error_Side<tol:#Conditional statements to find approximate value of rk and xt
                                sol_R = r_k[j]
                                sol_Xt = x_t[j]
                                return
