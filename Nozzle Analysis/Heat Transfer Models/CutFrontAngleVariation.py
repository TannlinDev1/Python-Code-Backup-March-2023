...
V = 40E-3 #Translational Cutting Speed (m/s)
...
P_Error = 1.01
theta_min =  np.arctan(X_t/d)               
theta_max = 20*np.pi/180
theta = np.linspace(theta_min,theta_max,num=100)
P_LHS = np.zeros(len(theta))
P_RHS = np.zeros(len(theta))
Surf_Temp = np.zeros(len(theta))
film_thickness = np.zeros(len(theta))
film_velocity = np.zeros(len(theta))
P_Ab_T = np.zeros(len(theta))
P_Ab_B = np.zeros(len(theta))
X_B = np.zeros(len(theta))

for I in range(0,len(theta)):
    
        k = 1
        X_b = X_t - d*np.tan(theta[I])

        F_0 = d*W_K*(np.pi/2)*P_g
        F_n = d*W_K*(np.pi/2)*rho_g*V_g**2*np.tan(theta[I])
        ...
        P_LHS[I] = P_ab_1+abs(P_ab_2)
        P_RHS[I] = P_mcs + P_m + P_La
        
        P_Ab_T[I] = P_ab_1
        P_Ab_B[I] = P_ab_2
        X_B[I] = X_b
        P_Error = P_RHS[I]/P_LHS[I]
...
plot1 = plt.figure(1)
plt.plot(theta*57.3,P_LHS,'r',label='Laser Power')
plt.plot(theta*57.3,P_RHS,'b',label='Heat Losses')
plt.legend()
plt.grid()
plt.xlabel("Cut Front Angle (deg)")
plt.ylabel("Power (W)")

plot2 = plt.figure(2)
plt.plot(theta*57.2,Surf_Temp)
plt.grid()
plt.xlabel('Cut Front Angle (deg)')
