...
Z_0_min = 50E-6 #Laser Beam Focal Position From Top of Material (m)
Z_0_max = 200E-6
Z_0 = np.linspace(Z_0_min,Z_0_max,num=7)
...
We = np.zeros(len(Z_0))
angle = np.zeros(len(Z_0))
Vm = np.zeros(len(Z_0))
T_L = np.zeros(len(Z_0))
S_m = np.zeros(len(Z_0))
Kerf_Width = np.zeros(len(Z_0))

for N in range(0,len(Z_0)):

    w = r_f0*np.sqrt(1+(-Z_0[N]/z_r)**2)
    w_b = r_f0*np.sqrt(1+((d-Z_0[N])/z_r)**2)
    ...
    Z_0_str = round(Z_0[N]*1000000,2)
    t_str = round(elapsed,2)
    
    print('Global Energy Balance Error = '+str(round(100*abs(P_Error-1),2))+'%')
    print('Iteration for Z_0 = ' +str(Z_0_str)+' um took ' +str(t_str)+ 's')
    print('-----------------------------------')
    
plot1 = plt.figure(1)
plt.plot(Z_0*1000000,We)
...
plot3 = plt.figure(3)
plt.plot(Z_0*1000000,S_m*1000000)
plt.xlabel('Gas Pressure (MPa)')
plt.ylabel('Melt Film Thickness (um)')
plt.grid()

plot4 = plt.figure(4)
plt.plot(Z_0*1000000,T_L)
plt.xlabel('Gas Pressure (MPa)')
plt.ylabel('Surface Temperature (K)')
plt.grid()

plot5 = plt.figure(5)
plt.plot(Z_0*1000000,angle*57.3)
plt.xlabel('Gas Pressure (MPa)')
plt.ylabel('Kerf Angle (deg)')
plt.grid()

plot6 = plt.figure(6)
plt.plot(Z_0*1000000,Vm)
plt.xlabel('Gas Pressure (MPa)')
